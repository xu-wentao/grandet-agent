...
func (s ProviderService) Test(ctx context.Context, name string) (domain.ProviderHealth, error) {
	...
	for _, config := range configs {
		...
		if !config.Enabled {
			return domain.ProviderHealth{}, fmt.Errorf("provider %q is disabled", name)
		}
		if config.Type != "openai_compatible" {
			return domain.ProviderHealth{}, fmt.Errorf("provider %q has unsupported type %q", name, config.Type)
		}
		apiKey := ""
		if config.APIKeyEnv != "" {
			var ok bool
			...
			if !ok || apiKey == "" {
				return domain.ProviderHealth{}, fmt.Errorf("provider %q requires environment variable %s", name, config.APIKeyEnv)
			}
		}
	...
	return domain.ProviderHealth{}, fmt.Errorf("provider %q is not configured", name)
}
